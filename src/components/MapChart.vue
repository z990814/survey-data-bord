...
<template>
  <div class="container">
    <div id="MapChartDiv" ref="MapChartDiv"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    drawMap() {
      this.$nextTick(() => {
        /* 获取地图数据*/
        this.MapChartDiv = echarts.init(document.getElementById("MapChartDiv"));
        var option = {
          series: [
            {
              tooltip: {
        ...
        this.MapChartDiv.setOption(option);
        window.onresize = this.MapChartDiv.resize;
        // 自动轮播
        autoToolTip(this.MapChartDiv, option, {
